$(document).ready(function () {
    ...
    $('#account-form').submit(function (event) {
        ...
        if (!first_name || first_name.length === 0) {
            ...
            block.removeBlockMessages().blockMessage(tjs.get('Password is missing'), {type: 'warning'});
        } else if (!password_verify || password_verify.length === 0) {
            block.removeBlockMessages().blockMessage(tjs.get('Password verification is missing'), {type: 'warning'});
