...
{
    let img_path_sgPermsByCat = source.img_path_sgPermsByCat;
    let sgid_sgPermsByCat = source.sgid_sgPermsByCat;
    ...
    $('.sg-set-perm').click(function(event){
        ...
        let options = {
            id: id,
            sid: sid,
            sgid: sgid_sgPermsByCat,
            block: block
        };

        if(src === img_path_sgPermsByCat + '/16/add2.png')
        {
            $(this).sgPermAdd(options, function(){
                    $(this).children().find('img').attr('src', img_path_sgPermsByCat + '/16/substract.png');
        ...
        {
            $(this).sgPermRem(options, function(){
                    $(this).children().find('img').attr('src', img_path_sgPermsByCat + '/16/add2.png');
                    ...
                    $(this).parent().parent('tr').css('opacity', 0.5).find('td').each(function()
                    {
                        ...
                        if(hasIcon){ icon_id_el.msDropDown().data('dd').set('disabled', true); }
                    });
                });
        }
    });
