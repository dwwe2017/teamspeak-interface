...
{
    let init_caching_channelGroupClientList = source.init_caching_channelGroupClientList;

    if(init_caching_channelGroupClientList === 1)
    {
    ...
    $('.modal-channel').click(function (event) {
        event.preventDefault();
        let cid = parseInt($(this).parent().prev().html(), 10);
        modalChannelModifyByCid(id, sid, cid);
    });
