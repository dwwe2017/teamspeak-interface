(function(d){d.fn.serverMsgSend=function(b,e){var a=d.extend({a:"",id:"",sid:"",message:"",block:this},b),c=d(a.block);if(a.id&&0!==a.id.length)if(a.sid&&0!==a.sid.length)if(a.message&&0!==a.message.length){(b="index.php?controller=ajax&action=serverMsgSend",""!==b)||(b=document.location.href.match(/^([^#]+)/)[1]);a={a:a.a,id:a.id,sid:a.sid,message:a.message};var f=(new Date).getTime();d.ajax({url:b,dataType:"json",type:"POST",data:a,success:function(a,b,d){a.valid?(a=a.response,b=(new Date).getTime(),
...
{type:"warning",position:"top"});else c.removeBlockMessages().blockMessage(tjs.get("Entity id is missing"),{type:"warning",position:"top"})}})(jQuery);
