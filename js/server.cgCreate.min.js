(function(b){b.fn.serverChannelgroupCreate=function(d){var a=b.extend({a:b("#a").val(),id:b("#id").val(),sid:b("#sid").val(),type:b("input[name=cgtype]:checked").val(),name:b("#cgname").val(),applypermsfrom:b("#applypermsfrom").val(),block:this},d),c=b(a.block);if(a.id&&0!==a.id.length)if(a.sid&&0!==a.sid.length)if(a.type&&0!==a.type.length)if(a.name&&0!==a.name.length)if(a.applypermsfrom&&0!==a.applypermsfrom.length){(d="index.php?controller=ajax&action=cgCreate",""!==d)||(d=document.location.href.match(/^([^#]+)/)[1]);
