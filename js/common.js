(function($)
{
	...
	$.fn.addTemplateSetup(function()
	{
		...
		this.find('input[type=radio].mini-switch, input[type=checkbox].mini-switch').each(function () {

			if(!$(this).next('span.mini-switch-replace').length) {
                $(this).hide().after('<span class="mini-switch-replace"></span>').next().click(function () {
                    $(this).prev().click();
                }).prev('.with-tip').next().addClass('with-tip').each(function () {
                    $(this).attr('title', $(this).prev().attr('title'));
                });
	 ...
	 * Equalize tab content blocks heights
	 */
	$.fn.equalizeTabContentHeight = function(additional)
	{
		var i;
		var g;
		var maxHeight;
		...
		for (i = this.length-1; i >= 0; --i)
		{
			...
			this.eq(i).find('a[href^="#"]').each(function(i)
			{
				...
				if (hash.length > 0)
				{
					...
					if (block.length > 0)
					{
						blockHeight = block.outerHeight()+parseInt(block.css('margin-bottom'))+additional;
						...
						{
							maxHeight = blockHeight+marginAdjustTop+marginAdjustBot+additional;
						}
	...
	$.fn.blockMessage = function(message, options)
	{
		...
		this.each(function(i)
		{
			...
			var finalMessage = (typeof message == 'object') ? '<ul class="'+messageClass+'"><li>'+message.join('</li><li>')+'</li></ul>' : '<p class="'+messageClass+'" '+style+'>'+message+'</p>';
			...
			else if (settings.position == 'current')
            {
                var children = block.find('.block-controls, .block-header, .wizard-steps');
                var lastHeader = children.first();
                var next = lastHeader.next('.message');
                var messageElement = lastHeader.after(finalMessage).next();
            }
			else
